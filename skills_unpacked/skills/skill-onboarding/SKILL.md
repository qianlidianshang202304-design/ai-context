...
2. **快速创建** — 使用官方脚手架工具初始化（推荐），或 AI 手写：
   ...
   **方式 A：脚手架初始化（推荐）**
   ```bash
   # 你的 skill 目录（按你用的工具选其一）：
   # Cursor：
   python3 scripts/init_skill.py <skill-name> --path ~/.cursor/skills
   # Claude Code：
   ...
   **方式 B：AI 手写最小版本**（不超过 20 行）
   - **确认 skill 目录**：先问用户用的是什么工具（Cursor / Claude Code / Codebuddy），然后创建到对应目录：
     - Cursor: `~/.cursor/skills/<name>/SKILL.md`
     - Claude Code: `~/.claude/skills/<name>/SKILL.md`
     - Codebuddy: `~/.codebuddy/skills/<name>/SKILL.md`
     - 或项目级：`.cursor/skills/<name>/SKILL.md` / `.claude/skills/<name>/SKILL.md`
   - 直接展示写好的文件内容给用户看
   - **逐行解释每一行的作用**，特别是：
...
1. **先展示一个带脚本的 skill 的目录结构**——用 onboarding 自身（在本 skill-onboarding 目录下运行）：
   ```bash
   ls -la .
   ls -la scripts/
   cat scripts/show-skills.sh
   ```
...
5. 找到质量合格的 skill 后，**Fork + 定制**：
   - 复制到用户的 skill 目录（Stage 1 确认过的路径）：`~/.cursor/skills/<新名字>/`（或 `~/.claude/skills/` / `~/.codebuddy/skills/`）
...
- **看你的 skills**：在 skill-onboarding 目录下 `bash scripts/show-skills.sh`，或 `ls ~/.cursor/skills/`
- **删除**：`rm -rf ~/.cursor/skills/skill-name/`（或对应的 .claude / .codebuddy 路径）
...
```bash
# 打包成 zip（自动验证格式）
python3 scripts/package_skill.py ~/.cursor/skills/<skill-name>/
```
...
- DO NOT 在 Stage 3 用 `ls ~/.cursor/skills/` 或 `ls ~/.claude/skills/` 列本地 skill 给用户选
